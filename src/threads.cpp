...
typedef struct CthThreadBase
{
  ...
  std::atomic<int> scheduled; /* has this thread been added to the ready queue ? */
...
CpvDeclare(char *,    CthData); /*Current thread's private data (externally visible)*/
...
{
  void *ret=NULL;
  if (*stackSize==0) *stackSize=CpvAccess(_defaultStackSize);

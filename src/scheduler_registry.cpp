...
void add_list_of_handlers(const std::vector<std::pair<QueuePollHandlerFn, unsigned int>>& handlers){
    ...
    for(const auto& handler : handlers){
        ...
        unsigned int step = SCHED_TABLE_SIZE / normalized;
        unsigned int index = current_index;
        while(remaining > 0){
            //find next empty slot
            if(total_assigned >= SCHED_TABLE_SIZE){
                break; // all slots assigned
            }
            while(CpvAccess(poll_handler_assigned)[index] != 0){
                index = (index + 1) % SCHED_TABLE_SIZE;
            }
            ...
            remaining--;
            index = (index + step) % SCHED_TABLE_SIZE;
        }
        current_index = (current_index + 1) % SCHED_TABLE_SIZE;
        handler_index++;
    }
}

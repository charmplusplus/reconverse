...
template <typename T> struct TemporaryObjectHolder<T, true> {
  typename std::remove_cv<typename std::remove_reference<T>::type>::type t{
      reconstruct()};
};
template <typename T> struct TemporaryObjectHolder<T, false> {
  typename std::remove_cv<typename std::remove_reference<T>::type>::type t{};
};
template <typename T> void operator|(er &p, TemporaryObjectHolder<T> &t) {
  p | t.t;
}
} // namespace detail
...
#ifndef CMK_PUP_LONG_LONG
static inline dataType getXlateDataType(long long *a) { return Tlonglong; }
static inline dataType getXlateDataType(unsigned long long *a) {
  return Tulonglong;
}
#else
static inline dataType getXlateDataType(CMK_PUP_LONG_LONG *a) {
  return Tlonglong;
}
static inline dataType getXlateDataType(unsigned CMK_PUP_LONG_LONG *a) {
  return Tulonglong;
}
#endif
#if CMK_HAS_INT16
...
public:
  ...
  // These state bits describe various user-settable properties. This needs to
  // be public because it used when PUPers are created from the checkpointing
  // and migration code.
  enum {
    IS_USERLEVEL =
    ...
    IS_CHECKPOINT =
        0x0020, // If set, it is creating or restarting from a checkpoint
  ...
  bool isSizing(void) const {
    return (PUP_er_state & IS_SIZING) != 0 ? true : false;
  }
  bool isPacking(void) const {
    return (PUP_er_state & IS_PACKING) != 0 ? true : false;
  }
  bool isUnpacking(void) const {
    return (PUP_er_state & IS_UNPACKING) != 0 ? true : false;
  }
  ...
  bool isDeleting(void) const {
    return (PUP_er_state & IS_DELETING) != 0 ? true : false;
  }
  ...
  bool isUserlevel(void) const {
    return (PUP_er_state & IS_USERLEVEL) != 0 ? true : false;
  }

  bool isCheckpoint(void) const { return (PUP_er_state & IS_CHECKPOINT) != 0; }

  bool isMigration(void) const { return (PUP_er_state & IS_MIGRATION) != 0; }

  bool hasComments(void) const {
    return (PUP_er_state & IS_COMMENTS) != 0 ? true : false;
  }

  // For single elements, pretend it's an array containing one element
  template <class T> void operator()(T &v) { (*this)(&v, 1); }

  void operator()(void *&v, void *sig) { (*this)(&v, 1, sig); }

  // For arrays:
  template <class T> void operator()(T *a, size_t nItems) {
    bytes((void *)a, nItems, sizeof(T), getXlateDataType(a));
  }

  // Standard pup_buffer API that calls malloc for allocation on isUnpacking and
  // free for deallocation on isPacking
  template <class T> void pup_buffer(T *&a, size_t nItems) {
    pup_buffer((void *&)a, nItems, sizeof(T), getXlateDataType(a));
  }

  // Custom pup_buffer API that calls user provided 'allocate' function for
  // allocation on isUnpacking and user provided 'deallocate' function for
  // deallocation on isPacking Custom pup_buffer behaves same as the standard
  // pup_buffer except for calling custom allocator and deallocator methods
  template <class T>
  void pup_buffer(T *&a, size_t nItems, std::function<void *(size_t)> allocate,
                  std::function<void(void *)> deallocate) {
    pup_buffer((void *&)a, nItems, sizeof(T), getXlateDataType(a), allocate,
               deallocate);
  }

  // For pointers: the last parameter is to make it more difficult to call
  //(should not be used in normal code as pointers may loose meaning across
  // processor)
  ...
  */
  ...
  // For allocatable objects (system will new/delete object and call pup
  // routine)
  void operator()(able **a) { object(a); }
  ...
  virtual void pup_buffer(void *&p, size_t n, size_t itemSize, dataType t,
                          ...
                          std::function<void(void *)> deallocate) = 0;
...
enum {
  ...
  sync_index =
      sync_builtin + 0x00120000, // Sync code for index of item in a list

  sync_last
};

/************** PUP::er -- Sizer ******************/
// For finding the number of bytes to pack (e.g., to preallocate a memory
// buffer)
...
template <class T> inline size_t size(T &t) {
  PUP::sizer p;
  p | t;
  return p.size();
}
...
public:
  ...
  inline void reset() { buf = origBuf; }

  inline void advance(size_t const offset) { buf += offset; }
};
...
template <class T> inline void toMemBuf(T &t, void *buf, size_t len) {
  PUP::toMem p(buf);
  p | t;
  if (p.size() != len)
    CmiAbort(
        "Size mismatch during PUP::toMemBuf!\n"
...
template <class T> inline void fromMemBuf(T &t, void *buf, size_t len) {
  PUP::fromMem p(buf);
  p | t;
  if (p.size() != len)
    CmiAbort("Size mismatch during PUP::fromMemBuf!\n"
             "This means your pup routine doesn't match during packing and "
             "unpacking");
}
...
public:
  ...
  toDisk(FILE *f, const unsigned int purpose = 0)
      : disk(IS_PACKING, f, purpose) {
    error = false;
  }
...
public:
  ...
  // You must close the file yourself when done.
  fromDisk(FILE *f, const unsigned int purpose = 0)
      : disk(IS_UNPACKING, f, purpose) {}
};
...
protected:
  virtual char *
  advance(char *cur) = 0; /*Consume current buffer and return next*/
...
public:
  wrap_er(er &p_, unsigned int newFlags = 0)
      : er(p_.getStateFlags() | newFlags), p(p_) {}
...
protected:
  typedef void (*dataConverterFn)(int N, const myByte *in, myByte *out,
                                  size_t nElem);
...
public:
  ...
  public:
    ...
    void pup(er &p) { p((char *)hash, sizeof(unsigned char) * len); }
    void pup(er &p) const { p((char *)hash, sizeof(unsigned char) * len); }
  };
...
#define PUPable_decl(className)                                                \
  PUPable_decl_inside(className) PUPable_operator_inside(className)
...
#define PUPable_decl_outside(className) PUPable_operator_outside(className)
...
#define PUPable_reg(className) className::register_PUP_ID(#className);
...
template <class T> class CkPointer {
...
template <class T> class CkReference : private CkPointer<T> {
public:
  /// Used on the send side, and does *not* delete the object.
...
*/
template <class T,
          typename std::enable_if<details::is_pupable<T>::value, int>::type = 0>
...
#define PUPbytes(type)                                                         \
  ...
  namespace PUP {                                                              \
  inline void PUParray(PUP::er &p, type *ta, size_t n) {                       \
    p((char *)ta, n * sizeof(type));                                           \
  }                                                                            \
  }                                                                            \
  namespace PUP {                                                              \
  template <> class as_bytes<type> {                                           \
  public:                                                                      \
    enum { value = 1 };                                                        \
  };                                                                           \
  }
...
#define PUPfunctionpointer(fnPtrType)                                          \
  inline void operator|(PUP::er &p, fnPtrType &t) {                            \
    p((char *)&t, sizeof(fnPtrType));                                          \
  }

/// Make PUP work with this enum type, copied as an "int".
#define PUPenum(enumType)                                                      \
  inline void operator|(PUP::er &p, enumType &e) {                             \
    int v = e;                                                                 \
    p | v;                                                                     \
    e = v;                                                                     \
  }
...
#define PUP_BUILTIN_SUPPORT(type)                                              \
  namespace PUP {                                                              \
  inline void operator|(er &p, type &t) { p(t); }                              \
  }                                                                            \
  namespace PUP {                                                              \
  inline void PUParray(er &p, type *ta, size_t n) { p(ta, n); }                \
  }                                                                            \
  namespace PUP {                                                              \
  template <> class as_bytes<type> {                                           \
  public:                                                                      \
...
#define PUPv(field, len)                                                       \
  do {                                                                         \
    if (p.hasComments())                                                       \
      p.comment(#field);                                                       \
    PUParray(p, field, len);                                                   \
  } while (0)

namespace PUP {
template <typename T> struct ptr_helper<T, true> {
  inline void operator()(PUP::er &p, T *&t) const {
    ...
    if (!is_nullptr) {
      ...
      p | t_able;
      if (p.isUnpacking())
        t = static_cast<T *>(t_able);
    }
  }
};

template <typename T> struct ptr_helper<T, false> {
  inline void operator()(PUP::er &p, T *&t) const {
    bool is_nullptr = nullptr == t;
    p | is_nullptr;
    if (!is_nullptr) {
...
protected:
  ...
  inline typename std::enable_if<
      std::is_constructible<U, reconstruct>::value>::type

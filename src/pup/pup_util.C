...
void PUP::er::operator()(able &a) { a.pup(*this); }

void PUP::er::comment(const char *message) { /* ignored by default */ }
...
void PUP::er::synchronize(unsigned int m) { /* ignored by default */ }
...
class pupCheckRec {
  ...
  int getMagic(void) const {
    return (magic[3] << 24) + (magic[2] << 16) + (magic[1] << 8) + magic[0];
  }
  void setMagic(int v) {
    for (int i = 0; i < 4; i++)
      magic[i] = (v >> (8 * i));
  ...
  /*Compare the packed value (from us) and the unpacked value
    (from the user).
   */
  void compare(const char *kind, const char *why, int packed,
               int unpacked) const {
    if (packed == unpacked)
      return;
...
public:
  ...
  void check(PUP::dataType t, size_t n) const {
    compare("magic number",
            "you unpacked more than you packed, or the values were corrupted "
            "during transport",
            getMagic(), pupMagic);
    compare("data type", "the pack and unpack paths do not match up", getType(),
            t);
    compare("length", "you may have forgotten to pup the array length",
            getLength(), n);
  }
};
#endif

void PUP::sizer::bytes(void * /*p*/, size_t n, size_t itemSize,
                       dataType /*t*/) {
#ifdef CK_CHECK_PUP
...
void PUP::fromMem::pup_buffer_generic(void *&p, size_t n, size_t itemSize,
                                      dataType t,
                                      std::function<void *(size_t)> allocate,
                                      bool isMalloc) {
#ifdef CK_CHECK_PUP
...
/*Disk PUP::er's*/
void PUP::toDisk::bytes(
    void *p, size_t n, size_t itemSize,
    dataType /*t*/) { /* CkPrintf("writing %d bytes\n",itemSize*n); */
...
void PUP::fromDisk::bytes(
    void *p, size_t n, size_t itemSize,
    dataType /*t*/) { /* CkPrintf("reading %d bytes\n",itemSize*n); */
  CmiFread(p, itemSize, n, F);
}

void PUP::fromDisk::pup_buffer(void *&p, size_t n, size_t itemSize,
                               dataType t) {
  if (isUnpacking())
    p = malloc(n * itemSize);
  bytes(p, n, itemSize, t);
...
//Code:
        ...
        s.endBlock();
*/
PUP::seekBlock::seekBlock(PUP::er &Np, int nSections) : nSec(nSections), p(Np) {
  if (nSections < 0 || nSections > maxSections)
    CmiAbort("Invalid # of sections passed to PUP::seekBlock!");
  p.impl_startSeek(*this);
  if (p.isPacking()) { // Must fabricate the section table
...
void PUP::wrap_er::impl_seek(seekBlock &s,
                             size_t off) /*Seek to the given offset*/
{
  p.impl_seek(s, off);
}
...
public:
  ...
  PUP_regEntry() : name(nullptr), ctor(nullptr) {} // Default constructor
  PUP_regEntry(const char *Nname, const PUP::able::PUP_ID &Nid,
...
const PUP_regEntry *PUP_getRegEntry(const PUP::able::PUP_ID &id,
  ...
  auto it = PUP_getRegistry()->find(id);
  const PUP_regEntry *cur =
      (it != PUP_getRegistry()->end()) ? &it->second : NULL;
  if (cur == NULL) {
    if (name_hint != NULL)
      CmiAbort("Unrecognized PUP::able::PUP_ID for %s", name_hint);
    CmiAbort(
        "Unrecognized PUP::able::PUP_ID. is there an unregistered module?");
  }
  return cur;
}
...
PUP::able::PUP_ID PUP::able::register_constructor(const char *className,
  ...
  (*PUP_getRegistry())[id] = PUP_regEntry(className, id, fn);
  return id;
}

PUP::able::constructor_function
PUP::able::get_constructor(const PUP::able::PUP_ID &id) {
...
void PUP::er::object(able **a) {
  ...
  if (isUnpacking()) {    // Find the object type & create the object
...
void PUP::toTextUtil::endLine(void) { cur = advance(cur); }
void PUP::toTextUtil::beginEnv(const char *type, int n) {
  char *const begin = beginLine();
  char *o = begin;
  snprintf(o, maxCount, "begin ");
  o += strlen(o);
  snprintf(o, maxCount - (o - begin), type, n);
  o += strlen(o);
...
void PUP::toTextUtil::pup_buffer(void *&p, size_t n, size_t itemSize,
                                 dataType t) {
  bytes(p, n, itemSize, t);
}
...
void PUP::toTextUtil::bytes(void *p, size_t n, size_t itemSize, dataType t) {
  if (t == Tchar) { /*Character data is written out directly (rather than
                       numerically)*/
    char *const begin = beginLine();
    char *o = begin;
    snprintf(o, maxCount, "string=");
    o += strlen(o);
    *o++ = '\"'; /*Leading quote*/
    ...
    for (size_t i = 0; i < n; i++) {
      if (c[i] == '\n') {
        ...
        snprintf(o, maxCount - (o - begin), "\\x%02X", (unsigned char)c[i]);
        o += strlen(o);
      } else if (c[i] == '\\' || c[i] == '\"') {
        snprintf(o, maxCount - (o - begin), "\\%c", c[i]);
        o += strlen(o);
      } else
        *o++ = c[i];
    }
    ...
    o += strlen(o);
    endLine();
  } else if (t == Tbyte || t == Tuchar) { /*Byte data is written out in hex
                                             (rather than decimal) */
    beginEnv("byte %d", n);
    ...
    char *o = begin;
    for (size_t i = 0; i < n; i++) {
      snprintf(o, maxCount - (o - begin), "%02X ", c[i]);
      o += strlen(o);
      if (i % 25 == 24 && (i + 1 != n)) { /* This line is too long-- wrap it */
        snprintf(o, maxCount - (o - begin), "\n");
        o += strlen(o);
        endLine();
        o = beginLine();
    ...
    endLine();
    endEnv("byte");
  } else { /*Ordinary number-- write out in decimal */
    if (n != 1)
      beginEnv("array %d", n);
    for (size_t i = 0; i < n; i++) {
      ...
      switch (t) {
      case Tshort:
        snprintf(o, maxCount, "short=%d;\n", ((short *)p)[i]);
        break;
      case Tushort:
        snprintf(o, maxCount, "ushort=%u;\n", ((unsigned short *)p)[i]);
        break;
      case Tint:
        snprintf(o, maxCount, "int=%d;\n", ((int *)p)[i]);
        break;
      case Tuint:
        snprintf(o, maxCount, "uint=%u;\n", ((unsigned int *)p)[i]);
        break;
      case Tlong:
        snprintf(o, maxCount, "long=%ld;\n", ((long *)p)[i]);
        break;
      case Tulong:
        snprintf(o, maxCount, "ulong=%lu;\n", ((unsigned long *)p)[i]);
        break;
      case Tfloat:
        snprintf(o, maxCount, "float=%.7g;\n", ((float *)p)[i]);
        break;
      case Tdouble:
        snprintf(o, maxCount, "double=%.15g;\n", ((double *)p)[i]);
        break;
      case Tbool:
        snprintf(o, maxCount, "bool=%s;\n", ((bool *)p)[i] ? "true" : "false");
        break;
#if CMK_LONG_DOUBLE_DEFINED
      case Tlongdouble:
        snprintf(o, maxCount, "longdouble=%Lg;\n", ((long double *)p)[i]);
        break;
#endif
#ifdef CMK_PUP_LONG_LONG
      case Tlonglong:
        snprintf(o, maxCount, "longlong=%lld;\n", ((CMK_PUP_LONG_LONG *)p)[i]);
        break;
      case Tulonglong:
        snprintf(o, maxCount, "ulonglong=%llu;\n",
                 ((unsigned CMK_PUP_LONG_LONG *)p)[i]);
        break;
#endif
      case Tpointer:
        snprintf(o, maxCount, "pointer=%p;\n", ((void **)p)[i]);
        break;
      default:
        CmiAbort("Unrecognized pup type code!");
      }
      endLine();
    }
    if (n != 1)
...
PUP::sizerText::sizerText(void)
    : toTextUtil(IS_SIZING + IS_COMMENTS, line, sizerText::lineLen),
      charCount(0) {}
...
PUP::toText::toText(char *outBuf, size_t len)
    : toTextUtil(IS_PACKING + IS_COMMENTS, outBuf, len), buf(outBuf),
      charCount(0) {}

/************** To/from text FILE ****************/
void PUP::toTextFile::pup_buffer(void *&p, size_t n, size_t itemSize,
                                 dataType t) {
  bytes(p, n, itemSize, t);
}
...
void PUP::toTextFile::bytes(void *p, size_t n, size_t itemSize, dataType t) {
  for (size_t i = 0; i < n; i++)
    ...
    case Tbyte:
      ...
      break;
    case Tshort:
      fprintf(f, " %d", ((short *)p)[i]);
      break;
    case Tushort:
      fprintf(f, " %u", ((unsigned short *)p)[i]);
      break;
    case Tint:
      fprintf(f, " %d", ((int *)p)[i]);
      break;
    case Tuint:
      fprintf(f, " %u", ((unsigned int *)p)[i]);
      break;
    case Tlong:
      fprintf(f, " %ld", ((long *)p)[i]);
      break;
    case Tulong:
      fprintf(f, " %lu", ((unsigned long *)p)[i]);
      break;
    case Tfloat:
      fprintf(f, " %.7g", ((float *)p)[i]);
      break;
    case Tdouble:
      fprintf(f, " %.15g", ((double *)p)[i]);
      break;
    case Tbool:
      fprintf(f, " %s", ((bool *)p)[i] ? "true" : "false");
      break;
#if CMK_LONG_DOUBLE_DEFINED
    case Tlongdouble:
      fprintf(f, " %Lg", ((long double *)p)[i]);
      break;
#endif
#ifdef CMK_PUP_LONG_LONG
    case Tlonglong:
      fprintf(f, " %lld", ((CMK_PUP_LONG_LONG *)p)[i]);
      break;
    case Tulonglong:
      fprintf(f, " %llu", ((unsigned CMK_PUP_LONG_LONG *)p)[i]);
      break;
#endif
    case Tpointer:
      fprintf(f, " %p", ((void **)p)[i]);
      break;
    default:
      CmiAbort("Unrecognized pup type code!");
    };
...
int PUP::fromTextFile::readInt(const char *fmt) {
  ...
  if (1 != fscanf(f, fmt, &ret)) {
    if (feof(f))
      return 0; /* start spitting out zeros at EOF */
    else
      parseError("could not match integer");
  }
  return ret;
}
unsigned int PUP::fromTextFile::readUint(const char *fmt) {
  ...
  if (1 != fscanf(f, fmt, &ret)) {
    if (feof(f))
      return 0u; /* start spitting out zeros at EOF */
    else
      parseError("could not match unsigned integer");
  }
  return ret;
}
CMK_TYPEDEF_INT8 PUP::fromTextFile::readLongInt(const char *fmt) {
  ...
  if (1 != fscanf(f, fmt, &ret)) {
    if (feof(f))
      ...
      parseError("could not match large integer");
  }
  return ret;
}
double PUP::fromTextFile::readDouble(void) {
  ...
  if (1 != fscanf(f, "%lg", &ret)) {
    if (feof(f))
      return 0.0; /* start spitting out zeros at EOF */
    else
      parseError("could not match double");
  }
  return ret;
}

void PUP::fromTextFile::pup_buffer(void *&p, size_t n, size_t itemSize,
                                   dataType t) {
  bytes(p, n, itemSize, t);
}
...
void PUP::fromTextFile::bytes(void *p, size_t n, size_t itemSize, dataType t) {
  for (size_t i = 0; i < n; i++)
    ...
    case Tuchar:
    case Tbyte:
      ((unsigned char *)p)[i] = (unsigned char)readInt("%02X");
      break;
    case Tshort:
      ((short *)p)[i] = (short)readInt();
      break;
    case Tushort:
      ((unsigned short *)p)[i] = (unsigned short)readUint();
      break;
    case Tint:
      ((int *)p)[i] = readInt();
      break;
    case Tuint:
      ((unsigned int *)p)[i] = readUint();
      break;
    case Tlong:
      ((long *)p)[i] = readInt();
      break;
    case Tulong:
      ((unsigned long *)p)[i] = readUint();
      break;
    case Tfloat:
      ((float *)p)[i] = (float)readDouble();
      break;
    case Tdouble:
      ((double *)p)[i] = readDouble();
      break;
#if CMK_LONG_DOUBLE_DEFINED
    case Tlongdouble: {
      long double ret = 0;
      if (1 != fscanf(f, "%Lg", &ret))
        parseError("could not match long double");
      ((long double *)p)[i] = ret;
    } break;
#endif
#ifdef CMK_PUP_LONG_LONG
    case Tlonglong: {
      ...
      if (1 != fscanf(f, "%lld", &ret))
        parseError("could not match long long");
      ((CMK_PUP_LONG_LONG *)p)[i] = ret;
    } break;
    case Tulonglong: {
      unsigned CMK_PUP_LONG_LONG ret = 0;
      if (1 != fscanf(f, "%llu", &ret))
        parseError("could not match unsigned long long");
      ((unsigned CMK_PUP_LONG_LONG *)p)[i] = ret;
    } break;
#endif
    case Tbool: {
      char tmp[20];
      if (1 != fscanf(f, " %19s", tmp))
        parseError("could not read boolean string");
      bool val = false;
      if (0 == strcmp(tmp, "true"))
        val = true;
      else if (0 == strcmp(tmp, "false"))
        val = false;
      else
        parseError("could not recognize boolean string");
      ((bool *)p)[i] = val;
    } break;
    case Tpointer: {
      void *ret = 0;
      if (1 != fscanf(f, "%p", &ret))
        parseError("could not match pointer");
      ((void **)p)[i] = ret;
    } break;
    default:
      CmiAbort("Unrecognized pup type code!");
...
void PUP::fromTextFile::comment(const char *message) {
  ...
  if (c != '!')
    return; // This isn't the start of a comment

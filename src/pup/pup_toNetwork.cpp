...
void PUP_toNetwork_sizer::bytes(void *p, size_t n, size_t itemSize,
                                PUP::dataType t) {
  switch (t) {
  ...
  case PUP::Tlongdouble:
    nBytes += n * 8;
    break;
  case PUP::Tpointer: // pointers are 8 bytes on 64-bit machines and 4 bytes on
                      // 32-bit machines
...
void PUP_toNetwork_sizer::pup_buffer(void *&p, size_t n, size_t itemSize,
                                     PUP::dataType t) {
...
void PUP_toNetwork_pack::bytes(void *p, size_t n, size_t itemSize,
                               PUP::dataType t) {
  size_t i;
  switch (t) {
  case PUP::Tchar: // Strings and bytes get copied as-is
...
#define casePUP_toNetwork_type(enumName, typeName, writeAs)                    \
  case PUP::enumName:                                                          \
    for (i = 0; i < n; i++)                                                    \
      w((writeAs)((uintptr_t)((typeName *)p)[i]));                             \
    break
...
void PUP_toNetwork_pack::pup_buffer(void *&p, size_t n, size_t itemSize,
                                    PUP::dataType t) {
  bytes(p, n, itemSize, t);
}
...
void PUP_toNetwork_unpack::bytes(void *p, size_t n, size_t itemSize,
                                 PUP::dataType t) {
  size_t i;
  switch (t) {
  case PUP::Tchar: // Strings and bytes get copied as-is
...
void PUP_toNetwork_unpack::pup_buffer(void *&p, size_t n, size_t itemSize,
                                      PUP::dataType t) {
  bytes(p, n, itemSize, t);
}

...
void CommBackendLCI1::init(int *argc, char ***argv) {
  ...
  LCI_SAFECALL(
      LCI_handler_create(LCI_UR_DEVICE, local_callback, &m_local_comp));
  LCI_SAFECALL(
      LCI_handler_create(LCI_UR_DEVICE, remote_callback, &m_remote_comp));
  LCI_plist_t plist;
...
int CommBackendLCI1::getMyNodeId() { return LCI_RANK; }

int CommBackendLCI1::getNumNodes() { return LCI_NUM_PROCESSES; }
...
void CommBackendLCI1::sendAm(int rank, void *msg, size_t size, mr_t mr,
  ...
  do {
    if (size <= LCI_MEDIUM_SIZE) {
      ...
      ret = LCI_putla(m_ep, buffer, m_local_comp, rank, remoteComp,
                      LCI_DEFAULT_COMP_REMOTE,
                      reinterpret_cast<void *>(localComp));
    }
...
void CommBackendLCI1::barrier(void) { LCI_SAFECALL(LCI_barrier()); }

...
void CommBackendLCI2::issueRget(int rank, void *local_buf, size_t size,
                                mr_t local_mr, uintptr_t remote_buf,
                                rkey_t rkey, CompHandler localComp) {
  lci::status_t status;
  do {
    status =
        lci::post_get_x(rank, local_buf, size, m_local_comp, remote_buf, rkey)
            .mr(local_mr)
            .user_context(reinterpret_cast<void *>(localComp))();
    lci::progress();
  } while (status.error.is_retry());
...
void CommBackendLCI2::issueRput(int rank, void *local_buf, size_t size,
  ...
  do {
    status =
        lci::post_put_x(rank, local_buf, size, m_local_comp, remote_buf, rkey)
            .mr(local_mr)
            .user_context(reinterpret_cast<void *>(localComp))();
    lci::progress();
  } while (status.error.is_retry());
...
rkey_t CommBackendLCI2::getRKey(mr_t mr_) { return lci::get_rkey(mr_); }

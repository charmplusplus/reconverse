...
thread_local CQdState cQdState;
...
int64_t CQdMsgGetCreated(CQdMsg msg)
{ CmiAssert(msg->phase==1); return msg->u.p1.created; }

void CQdMsgSetCreated(CQdMsg msg, int64_t c)
{ CmiAssert(msg->phase==1); msg->u.p1.created = c; }

int64_t CQdMsgGetProcessed(CQdMsg msg)
{ CmiAssert(msg->phase==1); return msg->u.p1.processed; }

void CQdMsgSetProcessed(CQdMsg msg, int64_t p)
...
int64_t CQdGetCreated(CQdState state)
{ return state->mCreated; }

void CQdCreate(CQdState state, int64_t n)
{ state->mCreated += n; }

int64_t CQdGetProcessed(CQdState state)
{ return state->mProcessed; }

void CQdProcess(CQdState state, int64_t n)
{ state->mProcessed += n; }
...
int64_t CQdGetCCreated(CQdState state)
{ return state->cCreated; }

int64_t CQdGetCProcessed(CQdState state)
{ return state->cProcessed; }

void CQdSubtreeCreate(CQdState state, int64_t c)
{ state->cCreated += c; }

void CQdSubtreeProcess(CQdState state, int64_t p)
{ state->cProcessed += p; }
...
{
  CQdProcess(cQdState, -1);
  CcdCallOnCondition(CcdPROCESSOR_STILL_IDLE, 
		     (CQdCondFn)CQdCallWhenIdle, (void*) msg);
}
